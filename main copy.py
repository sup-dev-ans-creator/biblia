...
class BookScreen(Screen):
    def __init__(self, biblia, **kwargs):
        super().__init__(**kwargs)
        self.biblia = biblia
        ...
        # Layout para o botão de lupa no topo
        top_layout = BoxLayout(orientation='horizontal', size_hint_y=None, height=50, padding=(0, 10))

        # Botão de lupa
        self.lupa_button = Button(
            size_hint=(None, 1), width=40,
            background_normal='', background_color=(0, 0, 0, 0)
        )
        self.lupa_button.add_widget(Image(source='lupa_icon.png', size=(32, 32)))  # Substitua pelo caminho correto do ícone
        self.lupa_button.bind(on_release=self.abrir_pesquisa)
        top_layout.add_widget(self.lupa_button)
        ...
        self.search_input.bind(text=self.filtrar_livros)
        top_layout.add_widget(self.search_input)

        # Adiciona o layout do topo ao layout principal
        self.layout.add_widget(top_layout)

        # ScrollView para exibir a lista de livros
        self.scroll_view = ScrollView(size_hint=(1, 1))
        self.book_list_layout = GridLayout(cols=1, size_hint_y=None, padding=(10, 10))
        self.book_list_layout.bind(minimum_height=self.book_list_layout.setter('height'))
        self.scroll_view.add_widget(self.book_list_layout)
        self.layout.add_widget(self.scroll_view)

        self.add_widget(self.layout)
        self.exibir_livros()  # Exibe todos os livros inicialmente

    def abrir_pesquisa(self, instance):
        # Alterna a visibilidade do campo de pesquisa ao clicar na lupa
        if self.search_input.opacity == 0:
            ...
            self.search_input.text = ""  # Limpa a pesquisa ao esconder

    def exibir_livros(self, filtro=""):
        # Limpa a lista atual de livros
        self.book_list_layout.clear_widgets()

        # Itera sobre os livros e exibe apenas os que correspondem ao filtro
        for livro in self.biblia.findall('b'):
            nome_livro = livro.attrib['n']
            if filtro.lower() in nome_livro.lower():
                livro_button = BookRow(abbreviation=livro.attrib.get('id', ''), name=nome_livro)
                livro_button.bind(on_release=partial(self.abrir_capitulos, livro.attrib.get('id'), nome_livro))
                self.book_list_layout.add_widget(livro_button)

    def filtrar_livros(self, instance, value):
        # Atualiza a lista de livros conforme o usuário digita
        self.exibir_livros(filtro=value)
...
class BibliaApp(App):
    def build(self):
        self.title = 'Bíblia e Plano de Leitura'
        sm = ScreenManager()
        ...
        # Carregar a Bíblia e passar para BookScreen
        biblia = carregar_biblia()
        book_screen = BookScreen(biblia=biblia, name='book_screen')
        sm.add_widget(book_screen)
